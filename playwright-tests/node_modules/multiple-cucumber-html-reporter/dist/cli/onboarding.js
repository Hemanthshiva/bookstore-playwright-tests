 ...
 */
export async function runOnboarding(cwd = process.cwd()) {
    p.intro('Welcome to Multiple Cucumber HTML Reporter!');
    p.note("No config file was found in the current directory.\nLet's create one so you can generate reports with a single command.", 'One-time setup');
    ...
    const options = {
        jsonDir: required.jsonDir.trim(),
        reportPath: required.reportPath.trim(),
    };
    // Report meta
    if (selected.includes('reportMeta')) {
        ...
        if ((meta.reportName || '').trim())
            options.reportName = meta.reportName.trim();
        if ((meta.pageTitle || '').trim())
            options.pageTitle = meta.pageTitle.trim();
        if ((meta.logo || '').trim())
            options.brandLogo = meta.logo.trim();
    }
    if (selected.includes('durations')) {
        ...
        options.displayDuration = dur.displayDuration;
        options.displayReportTime = dur.displayReportTime;
        options.durationInMS = dur.durationInMS;
        options.durationAggregation = dur.durationAggregation;
    }
    // Chart percentages
    if (selected.includes('charts')) {
        ...
        options.displayChartPercentages = chart;
    }
    if (selected.includes('browser')) {
        ...
        options.openReportInBrowser = openBrowser;
    }
    if (selected.includes('cdn')) {
        ...
        options.useCDN = useCDN;
    }
    if (selected.includes('logging')) {
        const logging = await p.select({
    ...
    if (selected.includes('metadataFilePath')) {
        ...
        if (mdfp.trim()) {
            options.metadataFilePath = mdfp.trim();
        }
    }
    if (selected.includes('metadata') && !options.metadataFilePath) {
        ...
        const metadata = {};
        if ((md.browserName || '').trim()) {
            metadata.browser = {
                name: md.browserName.trim(),
                version: (md.browserVersion || '').trim() || '',
            };
        }
        if ((md.platformName || '').trim()) {
            metadata.platform = {
                name: md.platformName.trim(),
                version: (md.platformVersion || '').trim() || '',
            };
        }
        if ((md.device || '').trim()) {
            metadata.device = md.device.trim();
        }
