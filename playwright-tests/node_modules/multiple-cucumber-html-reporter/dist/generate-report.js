...
import { applyMetadataAndHooks, enrichStepEmbeddings, escapeHtml } from './report-helpers.js';
import { createJsonArrayFileWriter, createJsonSuiteFileWriter, streamFeaturesFromFile, } from './streaming/json-stream.js';
const { size } = _;
...
async function generateReport(options) {
    ...
    if (Array.isArray(options.metadata) && !customMetadata) {
        throw new Error(`Invalid metadata format: you provided metadata as an array of { name, value } objects but did not set the "customMetadata" option to "true". Either change your metadata to a Metadata object / per-feature Record<string, Metadata>, or set "customMetadata: true" to enable custom key/value metadata.`);
    }
    // Validate attachmentLayout: only "modal" and "inline" are recognised (unset defaults to "modal")
    if (options.attachmentLayout !== undefined &&
        options.attachmentLayout !== 'modal' &&
        options.attachmentLayout !== 'inline') {
        throw new Error(`Invalid attachmentLayout: "${options.attachmentLayout}". Expected "modal" or "inline" (or leave it unset to use the default, "modal").`);
    }
    ...
    const allFeatures = await collectJSONS(options);
    ...
    function getReportRuntimeMetadata(suite) {
    ...
    function getCustomDataItems(customData) {
        if (!customData)
            return undefined;
     ...
     */
    function _parseSingleFeature(suite, feature, trackAggregates, mediaWriter) {
        feature.scenarios = {
            passed: 0,
            failed: 0,
            notDefined: 0,
            ...
            if (Array.isArray(feature.metadata)) {
                ...
                feature.metadata.forEach((item) => {
                    ...
                    if (label === 'device') {
                        ...
                        feature.executionPlatform = value;
                    }
                ...
                if (feature.metadata.executionPlatform)
                    feature.executionPlatform = feature.metadata.executionPlatform;
     ...
     * @private
     */
    function _parseScenarios(feature, trackAggregates, mediaWriter) {
     ...
     * @private
     */
    function _parseSteps(scenario, trackAggregates, mediaWriter) {
    ...
    async function _createFeatureIndexPages(suite) {
        ...
        let mergedWriter = null;
        let enrichedWriter = null;
 ...
 * Supports JSON.
 */
async function loadMetadataFile(filePath) {
